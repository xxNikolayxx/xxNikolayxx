...
class TestKeyElementsPresence:
    ...
    def test01_elements_code_auth(self, web_browser):
        page = KeyCodeAuthPage(web_browser)  # Загружаем страницу и проверяем её элементы
    ...
    def test02_elements_password_auth(self, web_browser):
        ...
        # assert page.rit_tagline.is_presented()  # элемента нет, скрываем проверку, баг заводим
...
class TestKeyCodeAuthReg:
    ...
    def test1_2_email_code_auth_pos(self, web_browser):
        page = KeyCodeAuthPage(web_browser)
        ...
        if page.btn_key_go.is_visible():
            page.btn_key_go.click()
        assert page.cabinet.get_text() == 'Личный кабинет'  # В этом месте тест падает, потому что Ключ не пускает
        # в кабинет, в котором не подключены услуги, при нажатии на "Войти" страница просто обновляется
    ...
    @pytest.mark.xfail(reason='Не тот код / в кабинет не попасть')
    def test1_5_email_code_autoreg_pos(self, web_browser):
        page = KeyCodeAuthPage(web_browser)
...
class TestKeyPassAuth:
    ...
    def test2_1_email_password_auth_pos(self, web_browser, email):
        page = KeyPasswordAuthPage(web_browser)
    ...
    @pytest.mark.parametrize("phone", [seven_phone_plus, eight_phone, seven_phone_without_plus],
                             ids=["+7x format", "8x format", "7x format"])
    @pytest.mark.xfail(reason='В кабинет не попасть')
    def test2_4_phone_password_auth_pos(self, web_browser, phone):
        page = KeyPasswordAuthPage(web_browser)
    ...
    @pytest.mark.xfail(reason='В кабинет не попасть')
    def test2_7_login_password_auth_pos(self, web_browser):
        page = KeyPasswordAuthPage(web_browser)
...
class TestKeyRegistration:
    ...
    @pytest.mark.xfail(reason='Не тот код / в кабинет не попасть')
    def test3_2_standard_reg_mailru_pos(self, web_browser):
        page = KeyRegPage(web_browser)
